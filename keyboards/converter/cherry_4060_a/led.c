...
static void set_value(int index, uint8_t value) {
  if (index >= 0 && index < LED_DRIVER_LED_COUNT) {
    led_matrix led = g_leds[index];
    led_states[led.matrix_co.row][led.matrix_co.col].set_value = value;
  }
}

static void set_value_all(uint8_t value) {
  for (uint8_t row = 0; row < MATRIX_ROWS; row++) {
    led_state_t *states = led_states[row];
...
led_state_t led_states[MATRIX_ROWS][MATRIX_COLS];
